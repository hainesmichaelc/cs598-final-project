...
def extract_data(use_ICU:str, label:str, time:int, icd_code:str, root_dir, disease_label, cohort_output=None, summary_output=None, limit=None, path="full"):
    ...
    if not os.path.exists(f"./data/{path}/cohort"):
        os.makedirs(f"./data/{path}/cohort")
    print("===========MIMIC-IV v2.0============")
    ...
    # sample from the cohort, if a limit was specified
    if limit:
        np.random.seed(12345)
        cohort = cohort.sample(limit)
    
    # save output
    cohort=cohort.rename(columns={"race":"ethnicity"})
    cohort[cols].to_csv(f"{root_dir}/{path}/cohort/{cohort_output}.csv.gz", index=False, compression='gzip')
    ...
    with open(f"./data/{path}/cohort/{summary_output}.txt", "w") as f:
        f.write(summary)
