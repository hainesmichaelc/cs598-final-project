...
class Generator():
    ...
    def __init__(self,cohort_output,if_mort,if_admn,if_los,feat_cond,feat_proc,feat_out,feat_chart,feat_med,impute,include_time=24,bucket=1,predW=6,path="full"):
        if not os.path.exists(f"./data/{path}/dict"):
            os.makedirs(f"./data/{path}/dict")
        if not os.path.exists(f"./data/{path}/csv"):
            os.makedirs(f"./data/{path}/csv")
        self.path = path
    ...
    def generate_adm(self):
        ...
        data=pd.read_csv(f"./data/{self.path}/cohort/{self.cohort_output}.csv.gz", compression='gzip', header=0, index_col=None)
    ...
    def generate_cond(self):
        cond=pd.read_csv(f"./data/{self.path}/features/preproc_diag_icu.csv.gz", compression='gzip', header=0, index_col=None)
    ...
    def generate_proc(self):
        proc=pd.read_csv(f"./data/{self.path}/features/preproc_proc_icu.csv.gz", compression='gzip', header=0, index_col=None)
    ...
    def generate_out(self):
        out=pd.read_csv(f"./data/{self.path}/features/preproc_out_icu.csv.gz", compression='gzip', header=0, index_col=None)
        out=out[out['stay_id'].isin(self.data['stay_id'])]
        out[['start_days', 'dummy','start_hours']] = out['event_time_from_admit'].str.split(' ', 
        expand=True)
    ...
    def generate_chart(self):
        chunksize = 5000000
        final=pd.DataFrame()
        for chart in tqdm(pd.read_csv(f"./data/{self.path}/features/preproc_chart_icu.csv.gz", compression='gzip', header=0, index_col=None,chunksize=chunksize)):
    ...
    def generate_meds(self):
        meds=pd.read_csv(f"./data/{self.path}/features/preproc_med_icu.csv.gz", compression='gzip', header=0, index_col=None)
...
#         else:
    ...
    def create_chartDict(self,chart,los):
        ...
        ######SAVE DICTIONARIES##############
        with open(f"./data/{self.path}/dict/metaDic", 'rb') as fp:
            metaDic=pickle.load(fp)
        
        with open(f"./data/{self.path}/dict/dataChartDic", 'wb') as fp:
            pickle.dump(dataDic, fp)

      
        with open(f"./data/{self.path}/dict/chartVocab", 'wb') as fp:
            pickle.dump(list(chart['itemid'].unique()), fp)
        self.chart_vocab = chart['itemid'].nunique()
        metaDic['Chart']=self.chart_per_adm
        
            
        with open(f"./data/{self.path}/dict/metaDic", 'wb') as fp:
...
#         print("# Unique insurance",self.data.insurance.nunique())
        ...
        for subdir, dirs, files in os.walk(rootdir):
            ...
            break
        for hid in tqdm(self.hids):
            if int(hid) > int(last_hid):
                grp=self.data[self.data['stay_id']==hid]
                demo_csv=grp[['Age','gender','ethnicity','insurance']]
                if not os.path.exists(f"./data/{self.path}/csv"+str(hid)):
                    try:
                        os.makedirs(f"./data/{self.path}/csv/"+str(hid))
                    except:
                        pass
                demo_csv.to_csv(f'./data/{self.path}/csv/'+str(hid)+'/demo.csv',index=False)
        ...
        #                 print(amount.columns)
                ...
                dyn_csv.to_csv(f'./data/{self.path}/csv/'+str(hid)+'/dynamic.csv',index=False)
                ...
                if(self.feat_cond):
                    ...
                    if(grp.shape[0]==0):
                        ...
                        grp=grp[feat]
                        grp.columns=pd.MultiIndex.from_product([["COND"], grp.columns])
                grp.to_csv(f'./data/{self.path}/csv/'+str(hid)+'/static.csv',index=False)   
                labels_csv.to_csv(f'./data/{self.path}/csv/labels.csv',index=False)    
        ...
        if(self.feat_cond):
            with open(f"./data/{self.path}/dict/condVocab", 'wb') as fp:
        ...
        if(self.feat_proc):    
            with open(f"./data/{self.path}/dict/procVocab", 'wb') as fp:
