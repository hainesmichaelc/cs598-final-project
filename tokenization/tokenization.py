...
class BEHRT_models():
    def __init__(self,data_icu,diag_flag,proc_flag,out_flag,chart_flag,med_flag,lab_flag,path="full"):
    ...
    def tokenization(self):
        ...
        demo_list = []
        cond_list = []
        labels =  pd.read_csv(f'./data/{self.path}/csv/labels.csv')
        first = True
        #labels = labels.iloc[:1, :]
        print("STARTING READING FILES.")
        for hadm in tqdm.tqdm(labels.itertuples(), total = labels.shape[0]):
            labs = pd.read_csv(f'./data/{self.path}/csv/' + str(hadm[1]) + '/dynamic.csv')
            demo = pd.read_csv(f'./data/{self.path}/csv/' + str(hadm[1]) + '/demo.csv')
            cond = pd.read_csv(f'./data/{self.path}/csv/' + str(hadm[1]) + '/static.csv')
            if first:
        ...
        with open(f'./data/{self.path}/dict/ethVocab', 'rb') as fp:
            ethVocab_l = pickle.load(fp)
            for i in range(len(ethVocab_l)):
                ethVocab[ethVocab_l[i]] = i

        with open(f'./data/{self.path}/dict/insVocab', 'rb') as fp:
