...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.eth_embeddings = nn.Embedding(config.eth_vocab_size, config.hidden_size)
    ...
    def forward(self, word_ids, age_ids=None, gender_ids=None, ins_ids=None, eth_ids=None, posi_ids=None, seg_ids=None, age=True):
        if seg_ids is None:
        ...
        eth_embed = self.eth_embeddings(eth_ids)
        posi_embeddings = self.posi_embeddings(posi_ids)

        if age:
            embeddings = word_embed + segment_embed + age_embed + gender_embed + ins_embed + eth_embed + posi_embeddings
        else:
            embeddings = word_embed + segment_embed + gender_embed + ins_embed + eth_embed + posi_embeddings
...
class BertModel(Bert.modeling.BertPreTrainedModel):
    ...
    def forward(self, input_ids, age_ids=None, gender_ids=None, ins_ids=None, eth_ids=None,  seg_ids=None,
        ...
        if eth_ids is None:
            eth_ids = torch.zeros_like(input_ids)
        if seg_ids is None:
        ...
        embedding_output = self.embeddings(input_ids, age_ids, gender_ids, ins_ids, eth_ids, seg_ids, posi_ids)
...
class BertForMLM(Bert.modeling.BertPreTrainedModel):
    ...
    def forward(self, input_ids, age_ids, gender_ids, ins_ids, eth_ids, posi_ids=None, seg_ids=None, attention_mask=None,
                masked_lm_labels=None):
        sequence_output, _ = self.bert(input_ids, age_ids, gender_ids, ins_ids, eth_ids, posi_ids, seg_ids, attention_mask,
...
class BertConfig(Bert.modeling.BertConfig):
    def __init__(self, config):
        ...
        self.eth_vocab_size = config.get('eth_vocab_size')
...
class DataLoader(Dataset):
    def __init__(self, dataframe, vocab, max_len, code='code'):
        ...
        self.eth = dataframe['eth']
        self.masks = dataframe['masks']
        self.vocab = vocab

    def __getitem__(self, index):
        """
        return: age, code, ins, eth, position, segmentation, mask, label
        """
        # extract data
        ...
        return torch.FloatTensor(code).to(torch.int64), torch.LongTensor(age).to(torch.int64), torch.LongTensor(gender).to(torch.int64), torch.LongTensor(
            ins).to(torch.int64), torch.LongTensor(eth).to(torch.int64), \
               torch.LongTensor(position).to(torch.int64), torch.LongTensor(segment).to(torch.int64), \
