...
class BertEmbeddings(nn.Module):
    ...
    def forward(self, word_ids, seg_ids=None,
                posi_ids=None):
...
class BertForEHRPrediction(Bert.modeling.BertPreTrainedModel):
    ...
    def forward(self, input_ids, seg_ids=None, posi_ids=None, attention_mask=None):
        _, pooled_output = self.bert(input_ids, seg_ids, posi_ids, attention_mask,
...
class DataLoader(Dataset):
    def __init__(self, dataframe, max_len, code='code', labels='labels'):
    ...
    def __getitem__(self, index):
        ...
        return torch.FloatTensor(code).to(torch.int64), \
               torch.FloatTensor(segment).to(torch.int64), torch.FloatTensor(position).to(torch.int64), \
