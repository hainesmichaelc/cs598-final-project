...
class train_behrt():
    def __init__(self,src, age, sex, ethni, ins, target_data, path="full"):
        ...
        if not os.path.exists(f"./data/{path}/eval/behrt"):
            os.makedirs(f"./data/{path}/eval/behrt")
        ...
        def train(trainload, valload, device):
            ...
            for e in range(train_params["epochs"]):
                ...
                if val_loss < best_val:
                    ...
                    save_model(model_to_save.state_dict(), f'./checkpoints/{path}/behrt')
                    best_val = val_loss
        ...
        def run_epoch(e, trainload, device):
            ...
            for step, batch in enumerate(trainload):
                ...
                zeros = torch.zeros_like(ethni_ids)
                ethni_ids = torch.where(ethni_ids < zeros, zeros, ethni_ids)
        ...
        def eval(_valload, saving, device):
            ...
            behrt.eval()
            if saving:
                with open(f"./data/{path}/eval/behrt/preds.csv", 'w') as f:
                    f.write('')
                with open(f"./data/{path}/eval/behrt/labels.csv", 'w') as f:
                    f.write('')
            ...
            for step, batch in enumerate(_valload):
                ...
                if saving:
                    with open(f"./data/{path}/eval/behrt/preds.csv", 'a') as f:
                        pd.DataFrame(logits.detach().cpu().numpy()).to_csv(f, header=False)
                    with open(f"./data/{path}/eval/behrt/labels.csv", 'a') as f:
